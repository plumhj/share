...
외부 시스템에서 HSOA 시스템으로 회원가입 및 로그인을 연동하기 위한 API입니다.
...
외부 시스템에서 HSOA 시스템으로 회원가입을 수행합니다.
...
| `workspaceID` | number | 생성된 워크스페이스 ID |
...
외부 시스템에서 HSOA 시스템으로 로그인을 수행합니다.
...
3. HSOA 시스템에서 토큰을 검증하고 자동 로그인 처리
...
```bash
curl -X POST 'https://api.example.com/api/external/signup' \
...
```bash
curl -X POST 'https://api.example.com/api/external/signin' \
  -H 'Content-Type: application/json' \
  -H 'X-API-Key: your-api-key-here' \
  -d '{
    "email": "user@example.com"
